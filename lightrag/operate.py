...
# Conditionally import prompts based on KG_LANGUAGE environment variable
# Set KG_LANGUAGE='CN' before importing lightrag to use Chinese prompts
_kg_language = os.getenv("KG_LANGUAGE", "EN").upper()
if _kg_language == "CN":
    try:
...
def chunking_by_token_size(
    ...
    # Helper function to count tokens using the provided tokenizer
    def tiktoken_len(text):
        tokens = tokenizer.encode(text)
        return len(tokens)

    # Convert token sizes to approximate character sizes for RecursiveCharacterTextSplitter
    # This is an approximation since token-to-character ratio varies
    chunk_size_chars = max_token_size
    overlap_size_chars = overlap_token_size

    # Initialize the text splitter with markdown-optimized separators
    from langchain.text_splitter import RecursiveCharacterTextSplitter

    # Use custom separators if split_by_character is provided
    if split_by_character:
        if split_by_character_only:
            # Only use the specified character for splitting
            separators = [split_by_character]
        else:
            # Use the specified character as primary separator, with fallbacks
            separators = [split_by_character, "\n\n", "\n", " ", ""]
    else:
        # Default markdown-optimized separators
        # separators = ["\n## ", "\n### ", "\n#### ", "\n\n", "\n", " ", ""]
        separators = [
            "\n## ",  # 最后考虑二级标题分割
            "\n### ",  # 其次考虑三级标题分割
    ...
    # Format the results to match the original function's output structure
    results: list[dict[str, Any]] = []

    for index, chunk in enumerate(text_chunks):
        # Calculate actual token count for this chunk
        actual_tokens = tiktoken_len(chunk.strip())
...
async def extract_entities(
    ...
    async def _process_single_content(chunk_key_dp: tuple[str, TextChunkSchema]):
        ...
        file_title = chunk_dp.get('full_doc_id', "unknown")
        assert file_title and file_title != "unknown"
        # Create cache keys collector for batch processing
        cache_keys_collector = []

        # Get initial extraction
        hint_prompt = entity_extract_prompt.format(
            **{**context_base, "input_text": content, 'title': file_title}
        )
...
async def kg_query(
    ...
    #     chunks_vdb,
    # )
    query_context = await _build_query_context(
        query,
        ll_keywords_str,
        hl_keywords_str,
        knowledge_graph_inst,
    ...
    # result = dict(
    #     context=context, entities=entities_context, relations=relations_context, chunks=text_units_context,
    #     entities_str=entities_str, relations_str=relations_str, text_units_str=text_units_str
    # )
    if query_param.only_need_retrieved_context_list:
        # 返回已检索到的上下文（仅当 context 存在且有 text_units_context 时），否则返回空列表
        return context if (context is not None and query_context.get("text_units_context")) else []
...
async def _build_query_context(
    ...
    # file_paths = set([x.get("file_path") for x in entities_context + relations_context if (x.get("file_path") and x.get("file_path") != "unknown_source")])
    ...
    # To preserve the influence of entity order,  entity-based chunks should not be deduplcicated by vector_chunks
    if final_node_datas:
...
"""
    ...
    # eye_rag
    output_result = dict(
        context=result, entities=entities_context, relations=relations_context, chunks=text_units_context,
        entities_str=entities_str, relations_str=relations_str, text_units_str=text_units_str
    )
...
async def _get_edge_data(
    ...
    # results = await relationships_vdb.query(
    #     keywords, top_k=query_param.top_k, ids=['Primary Angle-Closure Disease PPP']
    # )  # did not work even ids provided
    if not len(results):
        return [], []
...
async def naive_query(
        ...
        text_units_context.append(
            {
                ...
                "content": chunk["content"],
                "file_path": file_path,
                'source': ''.join(os.path.basename(file_path).split('.')[:-1]) if file_path and file_path != "unknown_source" else file_path,
            }
        )
    ...
    if query_param.only_need_retrieved_context_list:
        ...
        # ```
        # """
        # return text_units_str # [x['content'] for x in text_units_context]  # ['text_units_str']
